...
app.layout = html.Div([
    ...
    html.Div([
        ...
        # Div para mostrar información de alertas
        html.Div(id='alerts-jams', style={'textAlign': 'center', 'border': '2px solid yellow', 'padding': '10px', 'margin': '10px 0'}),

        # El div principal para el mapa de QGIS
        html.Div([
            html.Iframe(
                src='/assets/mapaqgis.html',  # Asegúrate de que el archivo exista en esta ubicación
                style={'height': '600px', 'width': '100%'}
            )
        ], style={'width': '70%', 'display': 'inline-block', 'verticalAlign': 'top'}),
        
        # Sección de eventos recientes y detalles
        html.Div([
...
def update_dashboard_and_isolate_legend_click(n_intervals):
    ...
    # Generar archivo GeoJSON de alertas para superponer en QGIS
    def save_alerts_geojson(processed_data):
        features = [
            {
                "type": "Feature",
                ...
                "properties": {
                    "evento": alert['evento'],
                    "street": alert['street'],
                    "nearest_intersection": alert.get("nearest_intersection", "N/A")
                }
            }
            for alert in processed_data['alert_coordinates']
        ]
        geojson_data = {"type": "FeatureCollection", "features": features}
        with open('assets/alerts.geojson', 'w') as geojson_file:
            json.dump(geojson_data, geojson_file)
    
    # Llamada para crear el archivo GeoJSON
    save_alerts_geojson(processed_data)

    # Actualización de alertas por tipo
    alerts_by_type = update_alerts_by_type_figure(processed_data)
    alerts_by_type.update_layout(height=400)

    # Detalles de eventos y lista de eventos recientes
...
def create_minimap(lat, lon, street=None, nearest_intersection=None, evento=None, date=None):
    if lat is None or lon is None:
        fig = go.Figure()
        fig.update_layout(title="Ubicación no disponible", height=300, width=300, margin=dict(l=0, r=0, t=0, b=0))
        return fig
    hover_text = f'Evento: {evento}<br>Calle: {street}<br>Intersección: {nearest_intersection}<br>Fecha: {date}'
